...
import { FaChartLine, FaHotel, FaUsers, FaUserShield } from "react-icons/fa";
...
const AdminOverview = () => {
    ...
    const calculateStats = () => {
      if (allUsersData && allUsersData.length > 0) {
        ...
        setStats({
          ...
          recentSignups: allUsersData.filter(
            (u) =>
              new Date(u.createdAt) >
              new Date(Date.now() - 30 * 24 * 60 * 60 * 1000),
          ).length,
          activeUsers: allUsersData.filter(
            (u) =>
              u.lastLogin &&
  ...
  const StatCard = ({ icon, title, value, color, link }) => (
    <Link to={link} className="hover:scale-105 transition-transform">
      <div
        className={`bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md border-t-4 ${color} hover:shadow-lg`}
      >
        <div className="flex items-center justify-between">
          ...
          <div
            className={`p-3 rounded-full ${color.replace("border", "bg")} bg-opacity-10`}
          >
            {icon}
          </div>
        </div>
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 p-6 transition-colors duration-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Link
          ...
          className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md border-l-4 border-blue-500 hover:shadow-lg transition-shadow"
        >
        ...
        <Link
          ...
          className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md border-l-4 border-green-500 hover:shadow-lg transition-shadow"
        >
          <h3 className="font-semibold text-gray-800 dark:text-white mb-2">
            Manage Properties
          </h3>
          <p className="text-sm text-gray-600 dark:text-gray-400">
            View and edit all listed properties
          </p>
        </Link>
        <Link
          to="/admin-panel/settings"
          className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md border-l-4 border-purple-500 hover:shadow-lg transition-shadow"
        >
          <h3 className="font-semibold text-gray-800 dark:text-white mb-2">
            System Settings
          </h3>
